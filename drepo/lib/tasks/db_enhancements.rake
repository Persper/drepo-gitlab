      ...
      conn.execute(%Q{
        ...
        for r in
            select
              ...
              'ALTER TABLE public.' || T.myTable || ' ADD COLUMN IF NOT EXISTS drepo_updated_at timestamp without time zone DEFAULT now() NOT NULL;' ||
      ...
      conn.execute(%Q{
        ...
        for r in
            select
              ...
              'ALTER TABLE #{schema}.' || T.myTable || ' ADD COLUMN IF NOT EXISTS drepo_updated_at timestamp without time zone NOT NULL;' ||
              'CREATE INDEX IF NOT EXISTS index_' || T.myTable || '_on_drepo_updated_at ON #{schema}.' || T.myTable || ' USING btree (drepo_updated_at);' ||
              'ALTER TABLE #{schema}.' || T.myTable || ' ADD COLUMN IF NOT EXISTS original_updated_at timestamp without time zone NOT NULL;' ||
              'CREATE INDEX IF NOT EXISTS index_' || T.myTable || '_on_original_updated_at ON #{schema}.' || T.myTable || ' USING btree (original_updated_at);' ||
              'ALTER TABLE #{schema}.' || T.myTable || ' ADD COLUMN IF NOT EXISTS original_created_at timestamp without time zone NOT NULL;' ||
