    ...
    class ProjectTreeRestorer
      ...
      def restore
        begin
          ...
          Rails.logger.error("Import/Export error: #{e.message} #{@path} #{File.exist?(@path)}")
          raise Gitlab::DrepoImportExport::Error.new('Incorrect JSON format')
        end
      ...
      def project_params
        @project_params ||= begin
          ...
          Gitlab::DrepoImportExport::AttributeCleaner.clean(relation_hash: attrs,
      ...
      def reader
        @reader ||= Gitlab::DrepoImportExport::Reader.new(shared: @shared)
      end
